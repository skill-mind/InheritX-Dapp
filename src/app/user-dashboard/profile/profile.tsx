...
const Profile: React.FC = () => {
  ...
  const [verificationStatus, setVerificationStatus] =
    useState<VerificationStatus>(VerificationStatus.NotStarted);
    ...
    switch (verificationStatus) {
              ...
              <div className="space-y-4">
                {profileFields.map((field, index) => (
                  <DirectionAnimation delay={0.1 * index} key={index}>
                    <div
                      key={field.label}
                      className="border-b border-gray-700 pb-2 flex"
                    >
                      <p className="text-sm text-white mb-1 flex-item-center w-1/2 md:w-1/3">
                        {field.label}
                      </p>
                      <p className="text-gray-300 font-extralight flex-item-center text-center">
                        {field.value}
                      </p>
                    </div>
                  </DirectionAnimation>
      ...
      case VerificationStatus.Pending:
        return showKyc ? (
          ...
          <PendingVerification {...profileData} />
        );
