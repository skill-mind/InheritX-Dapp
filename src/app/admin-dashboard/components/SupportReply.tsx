...
export default function ReplyModal({ isOpen, onClose }) {
  const [status, setStatus] = useState("reply"); 
  ...
  return (
    <div className="fixed inset-0 bg-[#0D0D11CC] bg-opacity-50 flex items-center justify-center">
      <div className="bg-[#21202A] p-8 rounded-3xl max-w-xl w-full h-[70%] text-white border border-[#615F71] ">
        ...
        {status === "reply" && (
          <button onClick={() => setStatus("deleted")} className="text-gray-400 text-xl hover:text-white">
            &larr; Go Back
          </button>
        )}
        ...
        {status === "sent" && (
          <>
            <div className="flex flex-col items-center justify-center h-full">
              ...
              <p className="text-center text-sm mt-4">Message sent to user's email address. Please continue discussion or <br /> settle disputes in mailbox.</p>
              <button 
                onClick={onClose} 
