        ...
        <div className="min-w-[800px]">
          ...
          <div className="divide-y divide-gray-800">
            {claimsData.map((claim) => (
              ...
              >
                <div className="text-xs md:text-sm truncate">{claim.planId}</div>
                <div className="text-xs md:text-sm truncate">{claim.planName}</div>
                <div className="text-xs md:text-sm truncate">{claim.assetOwner}</div>
                <div className="text-xs md:text-sm truncate">{claim.beneficiary}</div>
                <div>
                  <span className="text-xs md:text-sm">{claim.status}</span>
                </div>
                <div className="flex gap-2 md:gap-3">
                  <button className="text-indigo-500 hover:text-indigo-400 text-xs md:text-sm">View</button>
                  {claim.status !== "Rejected" && claim.status !== "Validated" && (
                    <button className="text-indigo-500 hover:text-indigo-400 text-xs md:text-sm">Validate</button>
                  )}
                  {claim.status !== "Rejected" && (
                    <button className="text-red-500 hover:text-red-400 text-xs md:text-sm">Reject</button>
                  )}
                  {claim.status === "Rejected" && (
                    <button className="text-indigo-500 hover:text-indigo-400 text-xs md:text-sm">Validate</button>
                  )}
