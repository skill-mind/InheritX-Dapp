...
const DataGroup = ({ title, subtitle, items }: DataGroupProps) => (
  <div>
    <h3 className="text-white font-bold text-lg md:text-xl lg:text-2xl mb-2 transition-all duration-300">{title}</h3>
    {subtitle && <p className="text-white font-normal text-sm md:text-base lg:text-lg mb-2">{subtitle}</p>}
    <ul className="space-y-1">
      {items.map((item, index) => (
        <li key={index} className="text-white font-normal text-sm md:text-base lg:text-lg flex items-center">
...
function PrivacyPolicy() {
  return (
    <section className='w-full px-4 md:px-8 lg:px-16'>
      <p className='font-bold text-2xl md:text-3xl lg:text-4xl text-center'>Privacy Policy</p>
