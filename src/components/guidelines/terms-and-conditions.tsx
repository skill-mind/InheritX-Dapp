...
//   return (
//     <div className='transition-all duration-300'>TermsAndConditions</div>
//   )
// }
...
export default function TermsAndCondition() {
  ...
  return (
    <section className="gap-[18px] px-4 md:px-8 lg:px-16">
      <h1 className="text-[25px] md:text-[40px] text-center font-black my-7 md:my-16">
        Terms and Conditions
      </h1>
      <div className="bg-gradient-to-b from-gray-300/20 via-gray-500/20 to-gray-700/20 rounded-[30px] md:rounded-[50px] border-[1px] border-[#413F54] py-[24px] md:py-[40px] px-[20px] md:px-[36px]">
        {termsData.map((section) => (
          <div
            key={section.id}
            className="mb-6 md:mb-8 border-b border-[#413F54] pb-6 md:pb-8"
          >
            <h2 className="text-[24px] md:text-[32px] font-bold mb-3 md:mb-4">
              {section.id}. {section.title || section.sections?.[0].title}
            </h2>
            {section.sections ? (
              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-[12px] md:gap-[18px]">
                {section.sections.map((subSection, index) => (
                  <div key={index} className="flex-1 gap-[6px] md:gap-[8px] ">
                    <h3 className="text-[20px] md:text-[24px] font-semibold mb-2 md:mb-3">
                      {subSection.title}
                    </h3>
                    <ul className="list-disc pl-4 md:pl-5 space-y-1 md:space-y-2">
                      {subSection.content.map((item, i) => (
                        <li
                          key={i}
                          className="text-[14px] md:text-[16px] text-[#FFFFFF] font-Manrope font-normal leading-[20px] md:leading-[21.86px] tracking-normal"
                        >
                          {item}
                        </li>
                      ))}
